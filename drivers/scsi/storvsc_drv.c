...
static int storvsc_channel_init(struct hv_device *device, bool is_fc)
{
	...
	printk("cdx:i storvsc_channel_init: max_chns=%d\n", max_chns);
...
static int storvsc_suspend(struct hv_device *hv_dev, pm_message_t state)
{
	struct storvsc_device *stor_device = hv_get_drvdata(hv_dev);
	struct Scsi_Host *host = stor_device->host;
	struct hv_host_device *host_dev = shost_priv(host);

	printk("cdx: storvsc_suspend: 111111111111111111111\n");
	...
	printk("cdx: storvsc_suspend: 111111111111111111111 --- done\n");
	//WARN_ON(1);
	return 0;
}
