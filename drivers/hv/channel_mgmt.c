...
static void vmbus_process_offer(struct vmbus_channel *newchannel)
{
	...
	if (is_hvsock_channel(newchannel) || newchannel->offermsg.offer.sub_channel_index > 0) {
		...
		printk("cdx: vmbus_process_offer: suspend_offer_in_progress=%d, inst=%pUl\n", atomic_read(&vmbus_connection.suspend_offer_in_progress), &newchannel->offermsg.offer.if_instance);
	}
	...
	if (fnew) {
		...
		printk("cdx: vmbus_process_offer: found a new primary channel: relid=%d\n",
			newchannel->offermsg.child_relid);
	}
	else {
		printk("cdx: vmbus_process_offer: found a new sub channel: relid=%d\n",
			newchannel->offermsg.child_relid);
		/*
		 ...
		 */
		printk("cdx: vmbus_process_offer: subchannel relid = %d, idx=%d\n",
			newchannel->offermsg.child_relid,
...
static void vmbus_onoffer(struct vmbus_channel_message_header *hdr)
{
	...
	printk("cdx: vmbus_onoffer: yicheng(new): relid = %d, type=%pUl, inst=%pUl\n",
		offer->child_relid, &offer->offer.if_type, &offer->offer.if_instance);
...
static void vmbus_onoffer_rescind(struct vmbus_channel_message_header *hdr)
{
	...
	printk("cdx: vmbus_onoffer_rescind: 3: relid=%d\n", rescind->child_relid);
	/*
	 ...
	 */
	printk("cdx: vmbus_onoffer_rescind: 4: relid=%d\n", rescind->child_relid);
	...
	printk("cdx: vmbus_onoffer_rescind: 5: relid=%d\n", rescind->child_relid);
	if (channel->device_obj) {
		...
		if (dev) {
			printk("cdx: vmbus_onoffer_rescind: 6.1: relid=%d\n", rescind->child_relid);
			vmbus_device_unregister(channel->device_obj);
			put_device(dev);
		}
	}
	if (channel->primary_channel != NULL) {
		 ...
		 */
		printk("cdx: vmbus_onoffer_rescind: 6.2: relid=%d\n", rescind->child_relid);
		mutex_lock(&vmbus_connection.channel_mutex);
		if (channel->state == CHANNEL_OPEN_STATE) {
			 ...
			 */
			printk("cdx: vmbus_onoffer_rescind: 6.2.1: relid=%d\n", rescind->child_relid);
			hv_process_channel_removal(channel);
		} else {
			printk("cdx: vmbus_onoffer_rescind: 6.2.2: relid=%d\n", rescind->child_relid);
	...
	if (complete_suspend_event) {
		printk("cdx: vmbus_onoffer_rescind: suspend_offer_in_progress=%d, inst=%pUl\n", atomic_read(&vmbus_connection.suspend_offer_in_progress), &channel->offermsg.offer.if_instance);
		if (atomic_dec_and_test(&vmbus_connection.suspend_offer_in_progress)) {
			printk("cdx: vmbus_onoffer_rescind: completing!!!\n");
			complete(&vmbus_connection.suspend_event);
		}
	}
}
...
{
	//XXX
	//bool complete_suspend_event = is_hvsock_channel(channel) || channel->offermsg.offer.sub_channel_index > 0;
